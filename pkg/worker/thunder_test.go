...
func TestThunderPreInitHookUsesCachedInstaller(t *testing.T) {
	manager := NewContainerThunderManager(nil)
	...
	hook, err := worker.thunderPreInitHook(&types.ContainerRequest{ContainerId: "container-123", GpuRequest: []string{"H100"}})
	if err != nil {
		t.Fatal(err)
	}
...
func TestThunderPreInitHookRequiresCachedInstaller(t *testing.T) {
	...
	if err == nil || !strings.Contains(err.Error(), "install command") {
		t.Fatalf("thunderPreInitHook() error = %v", err)
	}
}
